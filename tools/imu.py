...
@register_tool("IMU Raw")
class IMUTool(BaseTool):
    """
    Plots raw data from the IMU_ACCEL and IMU_GYRO messages.
    """
    ...
    def parse(self, *args, **kwargs):
        """
        Data is accessed directly in the plot method since it can come from two sources.
        """
        pass

    def plot(self):
        """
        Generates plots for raw body rates (gyro) and accelerations.
        """
        sns.set_theme(style="whitegrid")
        fig, axs = plt.subplots(2, 1, figsize=(15, 10), sharex=True, num="IMU Raw Data")
        fig.suptitle("IMU Raw Sensor Data", fontsize=16)
        ...
        if self.has_gyro:
            ...
            axs[0].set_title("Raw Gyroscope")
            axs[0].set_ylabel("Sensor Units")
            axs[0].legend()
            axs[0].grid(True)
        else:
            # If no data, display an empty plot with a clear title.
            axs[0].set_title("Raw Gyroscope - No Data")
            axs[0].grid(True)
        ...
        if self.has_accel:
            ...
            axs[1].set_title("Raw Accelerometer")
            axs[1].set_ylabel("Sensor Units")
            axs[1].legend()
            axs[1].grid(True)
        else:
            # If no data, display an empty plot with a clear title.
            axs[1].set_title("Raw Accelerometer - No Data")
            axs[1].grid(True)
